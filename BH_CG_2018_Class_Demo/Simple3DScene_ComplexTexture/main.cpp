...
void init_models(void)
{
	...
	g_models[3].mesh.CreateSphere(0.6f, 32, 16, 1.0f, 1.0f);
	...
	g_models[2].diffuse_color=vec4(1.0f, 1.0f, 1.0f, 1.0f);
	g_models[3].diffuse_color=vec4(1.0f, 1.0f, 1.0f, 1.0f);

	g_models[0].texture=LoadTexture2DFromFile("..\\textures\\wood01.jpg");
	g_models[1].texture=0;
	g_models[2].texture=LoadTexture2DFromFile("..\\textures\\metal01.jpg");
	g_models[3].texture=LoadTexture2DFromFile("..\\textures\\earth.jpg");
}
...
void display(void)
{
	...
	for (int i=0; i<4; ++i)
	{
		...
		glUniform1i(loc, g_models[i].texture!=0);

		if (g_models[i].texture!=0)
			glBindTexture(GL_TEXTURE_2D, g_models[i].texture);
